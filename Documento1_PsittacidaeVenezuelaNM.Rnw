...
gis.dir <- "/opt/gisdata/vectorial"
gis.dir <- "/media/jferrer/Intenso/gisdata/"
if (!exists("vz0"))
    vz0 <- shapefile(sprintf("%s/admin/ecoSIG_division_pol/division_pol.shp",gis.dir))
if (!exists("TMWB"))
    tmwb <- shapefile(sprintf("%s/admin/TMWB/TM_WORLD_BORDERS-0.3.shp",gis.dir))
...
if (!exists("mexp")) {
    e <- extent(rpre)
    mexp <- data.frame()
    gis.data <- sprintf("%s/distribucion/BirdLife/Psittacidae",gis.dir)
